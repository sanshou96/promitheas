...
function yahman_addons_Neatly_style(){
	...
	$css .= $wp_filesystem->get_contents(NEATLY_THEME_DIR . 'assets/css/style.min.css');

	if ( function_exists( 'has_blocks' ) ) {

		$css_url = NEATLY_THEME_DIR . 'assets/css/block.min.css';
		...
		if(!NEATLY_SIDEBAR){
			...
			$css_url = NEATLY_THEME_DIR . 'assets/css/block_one_column.min.css';
			if ( file_exists($css_url) )
				$css .= $wp_filesystem->get_contents( $css_url );
		}
	...
	$fontawesome = $wp_filesystem->get_contents(NEATLY_THEME_DIR .'assets/font/fontawesome/style.min.css');
	$fontawesome = str_replace('url(\'FontAwesome','url(\''.esc_url(NEATLY_THEME_URI) .'assets/font/fontawesome/FontAwesome',$fontawesome);
