  ...
  <div class="pf_img_wrap mb_M relative">
    <?php if($settings['image_bg'] != ''):
      ...
      <div class="pf_bg fit_box_img_wrap<?php echo $settings['fit_widget']; ?>">
        <?php
        echo '<'.( YA_AMP ? 'amp-img':'img decoding="async"').' src="'.esc_url($settings['image_bg']).'" height="160" width="310" alt="'.esc_html($settings['name']).'" />';
        ?>
      </div>
    ...
    if($settings['image'] != ''): ?>
     <div class="<?php echo esc_attr($has_bg); ?>">
      <div class="pf_img mla mra ta_c<?php echo esc_attr($has_bg_div);echo esc_attr($settings['image_shape']); ?>"<?php echo $has_bg_style; ?>>
        <?php
        echo '<'.( YA_AMP ? 'amp-img':'img decoding="async"').' src="'.esc_url($settings['image']).'" class="'.esc_attr($settings['image_shape']).'" height="120" width="120" alt="'.esc_attr( apply_filters('yahman_addons_profile_name', $settings['name'] ) ).'" />';
        ?>
      </div>
    </div>
    <?php
