...
class yahman_addons_google_cse_widget extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct(
			...
			array( 'description' => esc_html__( 'Support a Google Custom Search.Enable AMP when switching AMP form.', 'yahman-add-ons' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		if(YA_AMP){ ?>
			...
			<form method="GET" id="cse-search-box" action="<?php echo esc_attr($settings['action_url']); ?>" target="_blank">
				<input type="hidden" name="cx" value="<?php echo esc_attr($settings['cx']); ?>" />
				<input type="hidden" name="ie" value="UTF-8" />
				<input type="search" aria-label="<?php esc_attr_e( 'Search' , 'yahman-add-ons' ); ?>" placeholder="<?php esc_attr_e( 'Google Custom Search' , 'yahman-add-ons' ); ?>" name="q" required style="border:1px solid rgb(126,157,185);padding:2px;background:rgb(255,255,255);width:100%;height:32px;" />
				<input type="submit" value="Search" style="<?php echo $button_style; ?>" class="search-links" />
			</form>
    ...
    // Get Widget Settings.
		...
		<p>
			...
			<input class="ya_color-picker" id="<?php echo esc_attr( $this->get_field_id( 'icon_color' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'icon_color' ) ); ?>" type="text" value="<?php echo esc_attr( $settings['icon_color'] ); ?>" data-alpha="true" />
		</p>
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['icon_color'] = isset( $new_instance['icon_color'] ) ? esc_url( $new_instance['icon_color'] ) : '';
		return $instance;
	}
	...
	public function scripts($hook){
		...
		if ($hook == 'widgets.php' || $hook == 'customize.php') {
			...
			if(is_customize_preview()){
				...
				wp_enqueue_script('wp-color-picker-alpha',YAHMAN_ADDONS_URI . 'assets/js/customizer/wp-color-picker-alpha-widget.min.js', array('wp-color-picker'), null , true );
			}
