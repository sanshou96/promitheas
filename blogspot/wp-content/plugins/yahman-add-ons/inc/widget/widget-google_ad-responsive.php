...
class yahman_addons_google_ad_responsive_widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			array( 'description' => esc_html__( 'Support a responsive ad unit for Google AdSense.Enable AMP when switching AMP ad.', 'yahman-add-ons' ), ) // Args
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		while($i < 2):
			...
			if(YA_AMP){
				$amp_style['layout'] = '';
				$amp_style['height'] = 'height="320"'."\n";
				$amp_style['width'] = 'width="300"'."\n";
				$amp_style['auto-format'] = '';
				if($settings['format'] === 'vertical'){
					$amp_style['height'] = 'height="600"'."\n";
					$amp_style['width'] = '';
					$amp_style['layout'] = 'layout="fixed-height"'."\n";
				}else{
					if(wp_is_mobile()){
						$amp_style['width'] = 'width="100vw"'."\n";
						$amp_style['auto-format'] = "\n".'data-auto-format="rspv"
						data-full-width';
					}
				}
				echo apply_filters( 'widget_text',
					'<amp-ad '.$amp_style['layout'].
					$amp_style['width'].
					$amp_style['height'].
					'type="adsense"
					data-ad-client="'.$settings['client'].'"
					data-ad-slot="'.$settings['data_ad_slot'].'"'.$amp_style['auto-format'].'>
					<div overflow></div>
					</amp-ad>' );
			}else{
