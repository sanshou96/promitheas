...
if(YA_AMP){
	...
	$sns_info['icon_tooltip'] = '';
	$sns_info['opacity'] = ' sns_opacity';
}else{
	$out_put_style .= '#'.$sns_info['widget_id'].' .sns_icon_base{'.$sns_info['inline_style_base'].'}';
}

echo '<ul class="sns_link_icon f_box f_wrap'.esc_attr($sns_info['opacity']).esc_attr($sns_info['class']).' m0" style="list-style:none;">';
...
while($i++ < $sns_info['loop']){
	...
	$sns_info['hover_style'] = $sns_info['flickr_style'] = $sns_info['flickr_hover_style'] = $sns_info['amp_svg'] = $sns_info['amp_style'] = $sns_info['External_color'] = $sns_info['External_bg_color'] = $sns_info['inline_style'] = '';

	$sns_info['External_fill'] = 'fill:'.$sns_list[$sns_info['icon'][$i]]['color'].';';

	$sns_info['base_color'] = $sns_list[$sns_info['icon'][$i]]['color'];

	$sns_info['amp_svg_flickr_l'] = ' fill="#f40083"';
	$sns_info['amp_svg_flickr_r'] = ' fill="#006add"';
	...
	if($sns_info['need_border']){
		...
		if( 'flickr' !== $sns_info['icon'][$i] ){
			...
			if($sns_info['change_color'] === '10' || $sns_info['change_color'] === '11' ){
				$sns_info['External_color'] = 'color:'.$sns_info['icon_user_color'].';';
				$sns_info['amp_svg_flickr_l'] = $sns_info['amp_svg_flickr_r'] = ' fill="'.$sns_info['icon_user_color'].'"';
	...
	if($sns_info['need_bc']){
		$sns_info['External_fill'] = 'fill:#fff;';
			//$sns_info['amp_svg_flickr_l'] = $sns_info['amp_svg_flickr_r'] = ' fill="#fff"';
		if($sns_info['change_color'] === '00' || $sns_info['change_color'] === '01' ){
	...
	if(YA_AMP){
		...
		if($sns_info['need_border']){
			...
			if($sns_info['change_color'] === '00' || $sns_info['change_color'] === '01' ){
				$sns_info['amp_svg'] = ' fill="'.$sns_info['base_color'].'"';
				$sns_info['inline_style'] .= 'color:'.$sns_info['base_color'].';';
			}
			if($sns_info['change_color'] === '10' || $sns_info['change_color'] === '11' ){
				$sns_info['amp_svg'] = ' fill="'.$sns_info['icon_user_color'].'"';
				$sns_info['inline_style'] .= 'color:'.$sns_info['icon_user_color'].';';
				$sns_info['amp_svg_flickr_l'] = $sns_info['amp_svg_flickr_r'] = ' fill="'.$sns_info['icon_user_color'].'"';
			}
		...
		if($sns_info['need_bc']){
			$sns_info['amp_svg'] = $sns_info['amp_svg_flickr_l'] = $sns_info['amp_svg_flickr_r'] = ' fill="#fff"';

			if($sns_info['change_color'] === '00' || $sns_info['change_color'] === '01' ){
				$sns_info['inline_style'] .= 'background:'.$sns_info['base_color'].';';
			}
			if($sns_info['change_color'] === '10' || $sns_info['change_color'] === '11' ){
				$sns_info['inline_style'] .= 'background:'.$sns_info['icon_user_color'].';';
			}
		}
		...
		if($sns_info['icon_shape'] === "icon_character"){
			...
			if($sns_info['change_color'] === '10' || $sns_info['change_color'] === '11' ){
				$sns_info['amp_svg'] = $sns_info['amp_svg_flickr_l'] = $sns_info['amp_svg_flickr_r'] = ' fill="'.$sns_info['icon_user_color'].'"';
			}
	...
	if($sns_info['icon'][$i] != 'none' && $sns_info['url'][$i] != ''){
		...
		echo ' style="'. (YA_AMP ? $sns_info['inline_style_base'] : '') . $sns_info['inline_style'];
      //if(!YA_AMP) echo esc_attr($sns_info['hover_style']);
		echo '">';
		if( 'flickr' !== $sns_info['icon'][$i] ){
			echo '<svg class="svg-icon" '.$sns_info['svg_size'].$sns_info['amp_svg'].' viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">'.$sns_info['svg_radialGradient'].'<path class="sns_icon_'.$i.'" '.$sns_list[$sns_info['icon'][$i]]['svg'].'/></svg>';
		}else{
			echo '<svg class="svg-icon svg_flickr" '.$sns_info['svg_size'].$sns_info['amp_svg'].' viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path class="sns_icon_l_'.$i.'" '.$sns_list['flickr']['svg_L'].$sns_info['amp_svg_flickr_l'].'/><path class="sns_icon_r_'.$i.'" '.$sns_list['flickr']['svg_R'].$sns_info['amp_svg_flickr_r'].'/></svg>';
		}
