...
class yahman_addons_google_ad_in_feed_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		if ( $data['client'] != '' && ( ($data['slot'] != '' || !empty( $instance['data_ad_slot'] )) && ($data['layout_key'] != '' || !empty( $instance['data_ad_layout_key'] )) ) || ( ($data['slot_mobile'] != '' || !empty( $instance['data_ad_slot_mobile'] )) && ($data['layout_key_mobile'] != '' || !empty( $instance['data_ad_layout_key_mobile'] )) )){
			...
			if(YA_AMP){
				$data['slot_res'] = ! empty( $instance['data_ad_slot_res'] ) ? $instance['data_ad_slot_res'] : '';

				if ( $data['slot_res'] == ''){
					$data['slot_res'] =  isset($option['google_ad']['slot_responsive']) ? $option['google_ad']['slot_responsive'] : '';
				}
	...
	public function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'data_ad_slot_res' ) ); ?>"><?php echo esc_html_x('For AMP:', 'google_ad', 'yahman-add-ons'); ?><?php  echo sprintf(esc_html__( '%s ad unit\'s ID(data-ad-slot):', 'yahman-add-ons'),esc_html_x('Responsive', 'google_ad', 'yahman-add-ons')); ?></label>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'data_ad_slot_res' ) ); ?>"><?php esc_html_e( 'If empty this slot, then take precedence from YAHMAN Add-ons setting.', 'yahman-add-ons' ); ?></label>
			<label for="<?php echo esc_attr( $this->get_field_id( 'data_ad_slot_res' ) ); ?>"><?php esc_html_e( 'Enter either ,', 'yahman-add-ons' ); ?></label>
			<label for="<?php echo esc_attr( $this->get_field_id( 'data_ad_slot_res' ) ); ?>"><?php esc_html_e( 'Enable AMP when switching AMP ad.', 'yahman-add-ons' ); ?></label>
			<label for="<?php echo esc_attr( $this->get_field_id( 'data_ad_slot_res' ) ); ?>"><?php esc_html_e( 'If both empty,', 'yahman-add-ons' ); ?></label>
			<label for="<?php echo esc_attr( $this->get_field_id( 'data_ad_slot_res' ) ); ?>"><?php esc_html_e( 'Enable AMP when none ad.', 'yahman-add-ons' ); ?></label>
		</p>
