	...
	while($i < $match_count){
		...
		if(strpos($match_url[2][$i],home_url()) !== false){
			...
			if(isset($external_cache[$cache_name]['update_time'])){
				...
				//var_dump($diff_time);
				$has_cache = ( $diff_time > 60 ? false : true ) ;
			}
			...
			if ( $has_cache ){
				...
				$external_data = wp_remote_get( $match_url[2][$i] , array( 'timeout' => 50 , 'user-agent'  => ''  ) );
			...
			if($bc['cache_favicon'] !== '')
				$bc['cache_favicon'] = '<img src="'.esc_url( $bc['cache_favicon'] ).'" alt="'.$bc['title'].'" height="16" width="16" decoding="async" />';
		...
		<div class="bc_entry flex_70"><span class="bc_title line_clamp lc2 of_h fsM fw8 mb_S">'.esc_html($bc['title']).'</span><span class="bc_summary line_clamp lc2 of_h fsS mb_M">'.esc_html(strip_tags($bc['description'])).'</span>'.$bc['wordpress_dom'].'<div class="f_box ai_c jc_'.$bc['justify_content'].'">'.$bc['read_more'].'<div class="bc_info f_box ai_c fs10">'.$bc['cache_favicon'].'<span class="bc_domain">&nbsp;'.parse_url(esc_url($bc['link_url']), PHP_URL_HOST).'</span></div></div></div><div class="bc_thum flex_30 fit_box_img_wrap m0"><img src="'.$bc['img_source'].'" class="scale_13 trans_10 tap_no" alt="'.$bc['title'].'" width="3" height="2" decoding="async" /></div></a>';
...
function yahman_addons_blog_card_external($match_url,$bc,$external_data,$cache_name) {
	...
	if ( preg_match('/wordpress.org\/(themes||plugins)\/./', $match_url , $wordpress) ){
		...
		$external['head'] = $match_head[1];
		preg_match_all('/<meta property=[\'"]og:([^\'"]+)[\'"] content=[\'"]([^\'"]+)[\'"].*?>/is', $external['head'], $match['og']);
		preg_match_all('/<meta property=[\'"]fb:([^\'"]+)[\'"] content=[\'"]([^\'"]+)[\'"].*?>/is', $external['head'], $match['fb']);
		preg_match_all('/<meta name=[\'"]twitter:([^\'"]+)[\'"] content=[\'"]([^\'"]+)[\'"].*?>/is', $external['head'], $match['twitter']);

		if (isset($match['og']) && is_array($match['og']) && count($match['og']) == 3) {
			$j=0;
			while ($j < count($match['og'][1])) {
				if( !isset($tags['og:'.$match['og'][1][$j]]) )
					$tags['og:'.$match['og'][1][$j]] = esc_attr($match['og'][2][$j]);
				$j++;
			}
		}
		...
		if (isset($tags['og:description'])) {
			...
			if(preg_match('/<p.*?>(.*?)<\/p>/is', $external['html'], $bc['description'])){
