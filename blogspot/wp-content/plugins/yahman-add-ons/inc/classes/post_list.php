...
class YAHMAN_ADDONS_POST_LIST {
	...
	protected static function title_under_thumbnail($posts,$settings){
		...
		while ( $posts->have_posts() ) :
			...
			$ranking_data .= '<'.( YA_AMP ? 'amp-img':'img decoding="async"').' src="'.esc_url($thumurl[0]).'" class="scale_13 trans_10" width="'.esc_attr($thumurl[1]).'" height="'.esc_attr($thumurl[2]).'" alt="'.esc_attr($title).'" title="'.esc_attr($title).'" />';
	...
	protected static function title_over_thumbnail($posts,$settings){
		...
		while ( $posts->have_posts() ) :
			...
			$ranking_data .= '<div class="pl_thum_box fit_box_img_wrap">';
			$ranking_data .= '<'.( YA_AMP ? 'amp-img layout="responsive"':'img decoding="async"').' src="'.esc_url($thumurl[0]).'" class="" width="'.esc_attr($thumurl[1]).'" height="'.esc_attr($thumurl[2]).'" alt="'.esc_attr($title).'" title="'.esc_attr($title).'" />';
			$ranking_data .= '</div>';
	...
	protected static function list_with_thumbnail($posts,$settings){
		...
		while ( $posts->have_posts() ) :
			...
			$ranking_data .= '<'.( YA_AMP ? 'amp-img':'img decoding="async"').' src="'.esc_url($thumurl[0]).'" class="scale_13 trans_10" width="'.esc_attr($thumurl[1]).'" height="'.esc_attr($thumurl[2]).'" alt="'.esc_attr($title).'" title="'.esc_attr($title).'" /></div>';
