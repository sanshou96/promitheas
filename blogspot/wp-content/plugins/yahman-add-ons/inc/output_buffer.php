...
function yahman_addons_output_buffer_start($the_content){
	...
	if( defined('YA_AMP') && YA_AMP){

		$the_content = yahman_addons_amp_buffer_replace ( $the_content );
	...
	if( defined('YA_AMP') && YA_AMP){
		...
		if( isset($option['amp']['cache']) && is_singular() ){

			yahman_addons_make_cache( 'amp' , $the_content , $option['amp'] );

		}

	}else{

		if( isset($option['faster']['cache']) && is_singular() && empty($_GET['amp']) && !is_user_logged_in() ){
...
function yahman_addons_make_cache( $type , $the_content , $option ){
	...
	if( in_array ( $post_num , $post_not_in  ) ) {
	...
	}elseif( isset($option['cache_parent_not_in']) ){
		...
		if( in_array ( $post_num , explode(',', $option['cache_post_not_in'])  ) ) {
			$judge = false;
		}
	...
	if($judge)
		set_transient( 'ya_'.$type.'_cache_' . $post_num,
			$the_content,
